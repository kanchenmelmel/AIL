...
const TranscriptView = ({ transcript, display }) => (
    <div style={{ borderRadius: 6, background: '#EEEEEE', overflow: 'auto', height: display ? 180 : 1, overflowY: 'auto', transition: 'height 0.3s' }}>
        <pre style={{ margin: 6, fontSize: 14.5, fontWeight: 100, whiteSpace: 'pre-wrap' }}>{ transcript }</pre>
    </div>
);
...
const Title = ({ category, isAutoPlaying, onAutoPlayButtonClick }) => {
    ...
    switch (category) {
        ...
        case 'WriteFromDictation': title = 'Write from dictation'; break;
        case 'RetellLecture': title = 'Retell Lecture'; break;
        case 'SummariseSpokenText': title = 'Summarise Spoken Text'; break;
...
class Main extends React.Component {
    ...
    async componentDidMount() {
        ...
        const res = await fetch('http://ail.vic.edu.au/PTE真题音频/data.php');
        const data = await res.json();
        const category = data[this.props.category] ? this.props.category : 'ReadAloud';
...
ReactDOM.render(<Main category={window.CATEGORY}/>, document.getElementById('root'));
